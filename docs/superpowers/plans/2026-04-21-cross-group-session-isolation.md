...
gh pr create --title "fix: isolate peer sessions across groups" --body "$(cat <<'EOF'
## Summary
...
- 新增 `tests/session.test.ts`（9 个单元测试）

## Test plan
- [x] `bun test` 全绿（session 单元测试 + 既有 broker 集成测试）
...
- Fix C（迁移）→ Task 5 ✓
- 测试覆盖 → Task 1（跨组隔离 5 个）+ Task 5（迁移 4 个）✓
- 手动复现 → Task 6 Step 2 ✓
